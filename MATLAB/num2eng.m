...
%                       only the x-axis labels are processed, but the option 'axes' gives control over this - see line 229 of this comment box.
%
...
%                       shape of the input (see explanation of option cellOutput on line 143 for more info).
%
%                       Complex numbers are supported, but SI prefixes will not be used.
%
%       useSI:          Logical. When set to true, SI prefixes will be used for any number with magnitude in the range [1 q, 1000 Q), instead of scientific
%                       notation. Defaults to false if not specified.
%                       Examples:
%                       num2eng(0.001  ,true) returns '1 m'
%                       num2eng(2.56e49,true) returns '25.6e+48'
%                       num2eng(1.5    ,true) returns '1.5'
%                       num2eng(-1.2e5 ,1)    returns '-120 k'
%                       num2eng(87.2e31,true) returns '872 Q'
%                       num2eng(35e-28, true) returns '3.5 r'
%
...
%                       num2eng(2.56e35,true ,x    ,true) returns '256e+33 '
...
%                       num2eng(2.56e35     ,true,false,false,5,true) returns '256e+33'
%                       num2eng(1.5         ,true,false,false,5,true) returns '1.5'
%                       num2eng(4.5645667e-6,1   ,1    ,1    ,6,1)    returns '4.56457 micro'
%                       num2eng(87.2e31     ,true,false,false,5,true) returns '872 quetta'
%                       num2eng(35e-28      ,true,false,false,5,true) returns '3.5 ronto'
%
...
%
%   Using an options structure instead of individual option inputs (see from line 175 for the additional options available):
...
%       or the eleven additional options listed below (starting on line 175). You can use the function hints to remind you of the option names for naming of
...
%                       input axes are processed to format their tick labels. Simply list, in any order, the axes you wish to process. e.g. set to 'y' to
%                       process only the y-axis, or 'yz' or 'zy' to process y-axis and z-axis. Also, 'off' is a valid setting - this reverts any
%                       previously-processed labels to the default and stops listening for axis tick changes.
%                       Examples:
...
%   Developed with MATLAB R2018a running on Windows 10 and MATLAB R2023b macOS Sonoma. Inspired by a post by Jan Simon in the thread:
...
%% ==========================================================================================================================================================
    ...
    if opts.useSI
        expValue = 30:-3:-30;
        if opts.fullName
            prefix = {'quetta','ronna','yotta', 'zetta', 'exa', 'peta', 'tera', 'giga', 'mega', 'kilo', '', ...
                      'milli', 'micro', 'nano', 'pico', 'femto', 'atto', 'zepto', 'yocto','ronto','quecto'};
        else
            prefix = {'Q', 'R', 'Y', 'Z', 'E', 'P', 'T', 'G', 'M', 'k', '', 'm', 'u', 'n', 'p', 'f', 'a', 'z', 'y', 'r', 'q'};
            if opts.useMu, prefix{13} = char(956); end % char(956) is the lower case Greek "mu" character
        end
        for i=1:21, prefix{i} = [NO_BREAK_SPACE prefix{i}]; end
        if ~opts.spaceAfterNum, prefix{11} = ''; end
    else
        expValue = [];
        prefix   = [];
    end
...
%   7.0         -- 3 March 2024
%               -- Added support for SI prefixes Q (quetta, x10^30), R (ronna, x10^27), r (ronto, x10^-27), and q (quecto, x10^-30)
%
%============================================================================================================================================================
